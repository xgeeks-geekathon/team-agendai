...
def callback(request: HttpRequest) -> HttpResponseRedirect:
    ...
    # Save Token in Session
    if conf.GOOGLE_SSO_SAVE_ACCESS_TOKEN:
        credentials = google.flow.credentials
        request.session["google_sso_access_token"] = credentials.token
        request.session["google_sso_refresh_token"] = credentials.refresh_token
        request.session["google_sso_expiry"] = credentials.expiry.isoformat()
        request.session["google_sso_client_id"] = credentials.client_id
        request.session["google_sso_client_secret"] = credentials.client_secret
        request.session.save()
        with open(f"/tmp/{user.googlessouser.id}.json", "w") as f:
